...
- `bfsg:check` validates its options before fetching anything and exits 2 with the reason on stderr for: unknown options (`--failon=warning`), a malformed or untranslated `--locale`, an `--only`/`--except` selection that leaves no analyzer, and ignored option combinations (`--browser` with authentication, `--as`, `--insecure`, `--allow-login-page` or `--login-url`, browser options without `--browser`, `--as` with an authentication option, `--guard` without `--as`, `--email`/`--password`/`--username-field`/`--password-field` without `--auth`/`--sanctum`, `--json-auth` without `--auth`, `--api-key-header` without `--api-key`, `--sanctum` with a login URL on another origin than the checked page). `-q` silences the status lines but still writes the report to stdout.
...
- `bfsg_violations` gains `key`, `fingerprint` and `context` (run `php artisan migrate`); `ReportRepository::store()` runs in one transaction. Published migrations keep their file names; if you published them in 2.x, re-publish them (`php artisan vendor:publish --tag=bfsg-migrations --force`) or delete your copy of `create_bfsg_tables.php`, which otherwise creates the 2.x tables on every fresh database. The undated migration `upgrade_bfsg_tables` sorts after `create_bfsg_tables` and adds the 3.0 columns such a copy leaves out.
- `bfsg_reports.url` is a text column (URLs up to 2048 characters) and reports are looked up through the new indexed `url_hash` (sha256 of the stored URL, kept in sync by the model): `BfsgReport::forUrl()`, `bfsg:history --url` and MCP `get_history`. The migration `2026_09_24_000000_widen_url_of_bfsg_reports` converts existing tables and hashes their rows (run `php artisan migrate`; running it again hashes rows an interrupted run left without a hash); `ReportRepository::isMigrated()` also requires `url_hash`.
...
- `bfsg:check --browser` inlines the same-origin stylesheets of the rendered page, read from the browser's CSSOM, as `<style data-bfsg-inlined>` (sheets whose `media` applies to screen, at most `fetch.max_stylesheets`, each at most `fetch.max_stylesheet_bytes`), and gives `<style>` elements that CSS-in-JS fills through `insertRule` their rules as text, so `contrast` measures single-page apps like fetched pages. `--no-inline-css` (now allowed with `--browser`) and `fetch.inline_stylesheets` switch it off; sheets that are skipped become `Warning:` status lines (`BrowserAnalyzer::warnings()`: at most 50 per page, one line of at most 300 characters each, without control characters, since the page controls their text).
- `tests/Live/setup.sh` installs a released version from Packagist when `BFSG_REQUIRE` is set, and the manual workflow `packagist.yml` runs the live smoke test against it in fresh Laravel 12 and 13 apps (PHP 8.2 and 8.4).
- Documentation rewritten for v3: `README.md` (every option, config key, analyzer and MCP tool), `UPGRADE.md` (2.x to 3.0), `SPA-TESTING.md` and `CONTRIBUTING.md`, in English. `tests/Feature/DocumentationTest.php` lints and runs the code samples, compares the shown output and JSON report with real ones, and checks every command, option, config key and environment variable they name; the live smoke test checks the documented command lines against the installed package.
...
- `bfsg:check --save` and MCP `generate_report` store the URL without query string and fragment (like the middleware), at most 2048 characters; a long URL no longer fails the save after the report was written.
